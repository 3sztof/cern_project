...
class ApiTest:
    ...
    def __init__(self, opts):
        ...
        if typ.find('direct') >= 0 or typ.find('cli') >= 0:
            ...
            if opts.init and use_cli and self.opts.connection[:4] == 'http':
                ...
                c = TaskDB.connect(conn_typ, self.opts.connection)
                c.rebuild()
            elif opts.init:
                import Installer
            ...
            self.local = True
            self.api = TaskDB.connect(typ, self.opts.connection)
    ...
    def testGet(self, table = 'all'):
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Task_Sets'):
            ...
            result   = str(result).replace("u'",'"').replace("'",'"')
            success  = result == '[{"task_set": "' + itemName + '", "description": "bla"}]' and \
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Classes'):
            ...
            result   = str(result).replace("u'",'"').replace("'",'"')
            success  = result == '[{"node_class": "' + itemName + '", "description": "bla"}]' and self.inDb('Classes', node_class=itemName)
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Nodes'):
            ...
            result   = str(result).replace("u'",'"').replace("'",'"')
            success  = result == '[{"regex": "' + itemName + '", "description": "bla"}]' and \
    ...
    def testAssign(self, table = 'all'):
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Task_Sets'):
            ...
            self.direct.execStatement("insert into Task_Sets values ('{0}','')".format(itemName,))
            self.direct.execStatement("insert into Classes values ('{0}','')".format(setName,))
            result   = self.run_test(self.api.assignSet,task_set=itemName, node_class=setName)
            success  = result == SUCCESS and self.inDb('Task_Sets_to_Classes', task_set=itemName, node_class=setName)
            ...
            result   = self.run_test(self.api.assignSet,task_set=itemName, node_class=setName)
            success  = result != SUCCESS
            Test('assignSet', 'Assign an non-existing task set to an existing node class (or vice versa or both non-existing)').report(success)
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Classes'):
            ...
            result   = self.run_test(self.api.assignClass,node_class=itemName, node_regex=regexVal)
            ...
            result   = self.run_test(self.api.assignClass,node_class=itemName, node_regex=regexVal)
            success  = result != SUCCESS
            Test('assignClass', 'Assign an non-existing node class to an existing node(s) (or vice versa or both non-existing)').report(success)
    ...
    def testUnassign(self, table = 'all'):
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Tasks'):
            ...
            result   = self.run_test(self.api.unassignTask,task=itemName, task_set=setName)
            success  = result != SUCCESS
            Test('unassignTask', 'Unassign an non-existing task from an existing task set (or vice versa or both non-existing)').report(success)
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Task_Sets'):
            ...
            result   = self.run_test(self.api.unassignSet, task_set=itemName, node_class=setName)
            success  = result != SUCCESS
            Test('unassignSet', 'Unassign an non-existing task set from an existing node class (or vice versa or both non-existing)').report(success)
        ...
        if(table == 'all' or table == '*' or table == 'All' or table == 'Classes'):
            ...
            self.direct.execStatement("insert into Nodes values ('{0}','')".format(setName,))
            self.direct.execStatement("insert into Classes_to_Nodes values ('{0}','{1}')".format(itemName, setName))
            result   = self.run_test(self.api.unassignClass, node_class=itemName, node_regex=setName)
    ...
    def _log(self, function_name, function_description, result):
        self.testCount += 1
        tests_total_number = 55
        description_field_width = 100
        FORMAT = '%03d%-s%03d  %-13s  %-' + str(description_field_width) + 's  %-6s' # %-10s' # Possibly add timer
        logging.info(FORMAT % (self.testCount, '/', tests_total_number, function_name + (13 - len(function_name))*'.', 
