...
class Client:
    ...
    # \version 1.0
    # ---------------------------------------------------------------------------------------------------------------------
    def __init__(self, type, connection, debug=False, output=False):
        ...
        self.api    = implementation.connect(type, connection)
    ...
    def addTask(self, task, utgid='', command='', task_parameters='', command_parameters='', description=DSC_DEFAULT):
        ...
        if ( not self.batch and command == ''):
            if (self.yes_or_no('The required parameter - script name of the task has not been specified, do you want to do this now?', 'yes')):
                command = raw_input('Please provide the script name for your task: \n')
                print 'Success.' + self.os.linesep
        ...
        if ( not self.batch and command_parameters == ''):
            if (self.yes_or_no('The parameters string for the task has not been specified, do you want to do this now?', 'no')):
                command_parameters = raw_input('Please provide the parameters for your script in the line below: \n')
                print 'Success.' + self.os.linesep
        if ( not self.batch and task_parameters == ''):
            if (self.yes_or_no('No special parameters for the pcAdd command has been specified, do you want to do this now?', 'no')):
                task_parameters = raw_input('Please provide special parameters in the line below: \n')
                print 'Success.' + self.os.linesep
    ...
    def addSet(self, task_set, description=DSC_DEFAULT):
        if ( not self.batch and  description == DSC_DEFAULT):
            if(self.yes_or_no('The description of the task set has not been specified, do you want to do this now?', 'no')):
                description = raw_input('Please provide the description: \n')
                print 'Success.' + self.os.linesep
    ...
    def assignClass(self, node_class, node_regex):
        return self.do_call('assignClass',node_class=node_class,node_regex=node_regex)
    ...
    def unassignClass(self, node_class, node_regex):
        return self.do_call('unassignClass',node_class=node_class,node_regex=node_regex)
...
if __name__ == "__main__":
    ...
    parser.add_option("-o", "--options",
                      dest="arguments",
                      default='',
                      help="action arguments as a single string. Need to be quotes: \"<text>\"",
                      metavar="<string>")
    (opts, args) = parser.parse_args()
    
    #if opts.debug:
    #    print '%s %-16s %s'%('Action:', opts.action, opts.arguments, )
    ...
    if connl[:4] == 'http':
        ...
        cl = Client(typ, conn, debug=opts.debug, output=opts.output)
    elif connl[:7] == 'sqllite':
        cl = Client('direct', conn, debug=opts.debug, output=opts.output)
    else:
        logging.error('Unknown TaskDB connection:'+conn)
        sys.exit(errno.EINVAL)
    try:
        attr = getattr(cl,opts.action)
        cmd  = 'attr('+str(opts.arguments)+')'
        result = eval(cmd)
