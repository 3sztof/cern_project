...
import types, xmlrpc, string, random
...
#from http.client   import HTTP           as HTTP
class HTTP:
    pass
...
# Library includes
from jsonrpc  import config
from jsonrpc import history
...
def jdumps(obj, encoding='utf-8'):
    ...
    if cjson:
        ...
        return json.dumps(obj).encode(encoding=encoding)
...
class JSONTarget(object):
    ...
    def close(self):
        ...
        if isinstance(self.data,list):
            d = []
            for e in self.data:
                if isinstance(e,bytes): d.append(e.decode())
                else: d.append(e)
            self.data = d
...
class Transport(TransportMixIn, XMLTransport):
    def __init__(self, use_datetime=0):
        ...
        XMLTransport.__init__(self, use_datetime=use_datetime)


class SafeTransport(TransportMixIn, XMLSafeTransport):
    def __init__(self, use_datetime=0, context=None):
        TransportMixIn.__init__(self)
        XMLSafeTransport.__init__(self,use_datetime=use_datetime, context=context)
...
class ServerProxy(XMLServerProxy):
    ...
    def __init__(self, uri, transport=None, encoding=None,
                 verbose=0, version=None, use_datetime=0, context=None):
...
def dumps(
    ...
    if not version:
        version = config.version
    valid_params = (tuple, list, dict, str, int, float, bool)
    if type(methodname) != str and \
