 ...
 *
 * Revision 1.4  2008/10/22 09:41:28  galli
 * minor changes
 *
 * Revision 1.2  2007/12/19 14:33:38  galli
 * Can get DIM_DNS_NODE from: (1) cmd-line option, (2) environment (3) file.
 *
 * Revision 1.1  2007/09/05 09:16:00  galli
 * Initial revision
 */
/*****************************************************************************/
#include <stdio.h>
...
/*****************************************************************************/
int deBug=0;
static char rcsid[]="$Id: irqReset.C,v 1.5 2012/11/29 15:52:27 galli Exp galli $";
char *rcsidP;
const char *dimConfFile=DIM_CONF_FILE_NAME;
const char *srvName="IRQ";
/*****************************************************************************/
void usage(void);
/*****************************************************************************/
    ...
    while((type=br.getNextService(cmd,format))!=0)        /* loop over nodes */
    {
      if(!fnmatch(cmdPatt,cmd,0))
      {
        ...
        *p='\0';
        ...
        printf("    NODE: \"%s\". Resetting IRQ counters...\n",nodeFound);
